    ...
    <div
      ...
      <el-form :model="form" size="normal" style="height: 100% ;width: 220px;" ref="form" :rules="rules">
        <div style="width: 100%;height: 40px;margin-top:10px;line-height: 40px;text-align: center">
          <span style="color: rgb(79, 125, 242);font-weight: 900;font-size: 30px;margin-bottom: 10px;">欢迎注册</span>
        </div>
        <el-form-item style="margin-top: 20px" prop="buyername">
  ...
  methods: {
    ...
    register() {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          request.post("/api/buyer/register", this.form).then(res => {
            console.log(res);
            if (res.code === '0') {
              this.$message({
                type: "success",
